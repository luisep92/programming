...
namespace Luisbreria
{
    public class Renderer : Component
    {
        ...
        public rgba_f64 color = Color.white;
        float trail = 0;
        ...
        public void RenderRectangle(ICanvas canvas, GameObject go, rgba_f64 color)
        {
            FillRectangle(
                canvas,
        ...
        public void Render(ICanvas canvas)
        {
            ...
            canvas.FillShader.SetImage(this.sprite, new rgba_f64(color.r, color.g, color.b, color.a));
            canvas.Transform.SetTranslation((double)t.position.x - t.size.x / 2, (double)t.position.y - t.size.y / 2);
            canvas.DrawRectangle(new rect2d_f64(0, 0, (double)t.size.x, (double)t.size.y));
        }
        void RenderTrail(ICanvas canvas, Vector2 pos, float alpha)
        {
            Transform t = this.gameObject.transform;
            canvas.FillShader.SetImage(this.sprite, new rgba_f64(color.r, color.g, color.b, alpha));
            canvas.Transform.SetTranslation(0.0, 0.0);
            canvas.DrawRectangle(new rect2d_f64((double)pos.x - t.size.x / 2, (double)pos.y - t.size.y / 2, (double)t.size.x, (double)t.size.y));
        }
        void UpdateTrailPos()
        {
            if (trail >= 0.05f)
